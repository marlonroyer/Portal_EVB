...
function ajustarFonteParaConteudo(etiqueta) {
    ...
    const cabecalhos = etiqueta.querySelectorAll('.id');
    const titulos = etiqueta.querySelectorAll('.titulo-coluna');
    
    // Tamanhos de fonte expandidos até 5px
    const tamanhosFonte = [15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5, 4];
    const tamanhosCabecalho = [13, 12, 11, 10, 9, 8, 7, 6, 5, 4];
    const tamanhosTitulo = [11, 10, 9, 8, 7, 6, 5, 4];
    
    // SALVAR estilos originais
    const overflowOriginalFamilia = colunaFamilia.style.overflowY;
    const overflowOriginalIntervalos = colunaIntervalos.style.overflowY;
    
    // REMOVER overflow para detectar transbordamento real
    colunaFamilia.style.overflowY = 'visible';
    colunaIntervalos.style.overflowY = 'visible';
    
    function precisaReduzir(elemento) {
        // Agora vai detectar overflow real pois removemos o scroll
        const overflow = elemento.scrollHeight > elemento.clientHeight;
        console.log(`scrollHeight: ${elemento.scrollHeight}, clientHeight: ${elemento.clientHeight}, overflow: ${overflow}`);
        return overflow;
    }
    
    function aplicarTamanho(elemento, tamanho, propriedade = 'fontSize') {
        if (propriedade === 'fontSize') {
            elemento.style.fontSize = tamanho + 'px';
            elemento.style.lineHeight = (tamanho * 1.1) + 'px'; // Ainda mais apertado
        }
        console.log(`Aplicando ${propriedade}: ${tamanho}px em ${elemento.className}`);
    }
    
    function ajustarElemento(elemento, tamanhosArray) {
        for (let tamanho of tamanhosArray) {
            aplicarTamanho(elemento, tamanho);
            if (!precisaReduzir(elemento)) {
                console.log(`✅ Funcionou com ${tamanho}px`);
                return true;
            }
        }
        console.log(`❌ Não coube nem com ${tamanhosArray[tamanhosArray.length-1]}px`);
        return false;
    }
    
    // Ajusta conteúdo principal
    let familiaOk = ajustarElemento(colunaFamilia, tamanhosFonte);
    let intervalosOk = ajustarElemento(colunaIntervalos, tamanhosFonte);
    
    // Se ainda não coube, reduz TUDO
    if (!familiaOk || !intervalosOk) {
        console.log('🔴 Conteúdo extremo! Reduzindo tudo...');
        
        // Reduz cabeçalhos
        cabecalhos.forEach(cabecalho => {
            ajustarElemento(cabecalho, tamanhosCabecalho);
        });
        
        // Reduz títulos
        titulos.forEach(titulo => {
            ajustarElemento(titulo, tamanhosTitulo);
        });
        
        // Remove TODO padding e margens
        colunaFamilia.style.padding = '0';
        colunaIntervalos.style.padding = '0';
        etiqueta.style.padding = '0';
        
        // Remove bordas e margens dos elementos internos
        const todosElementos = etiqueta.querySelectorAll('*');
        todosElementos.forEach(el => {
            el.style.margin = '0';
            el.style.padding = '0';
        });
        
        // Se mesmo assim não coube, força 4px em tudo
        if (!familiaOk || !intervalosOk) {
            colunaFamilia.style.fontSize = '4px';
            colunaFamilia.style.lineHeight = '4.5px';
            colunaIntervalos.style.fontSize = '4px';
            colunaIntervalos.style.lineHeight = '4.5px';
            cabecalhos.forEach(el => {
                el.style.fontSize = '4px';
                el.style.lineHeight = '4.5px';
            });
            titulos.forEach(el => {
                el.style.fontSize = '4px';
                el.style.lineHeight = '4.5px';
            });
            console.log('⚡ Forçado 4px em tudo!');
        }
    }
    
    // RESTAURAR overflow original
    colunaFamilia.style.overflowY = overflowOriginalFamilia || 'auto';
    colunaIntervalos.style.overflowY = overflowOriginalIntervalos || 'auto';
